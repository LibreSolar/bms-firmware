...
{
    return !bms_chg_error(status->error_flags & ~BMS_ERR_CHG_OFF) && !status->full
           && status->chg_enable;
}

bool bms_dis_allowed(BmsStatus *status)
{
    return !bms_dis_error(status->error_flags & ~BMS_ERR_CHG_OFF) && !status->empty
           && status->dis_enable;
}
...
bool bms_balancing_allowed(BmsConfig *conf, BmsStatus *status)
{
    ...
    float voltage_diff = status->cell_voltage_max - status->cell_voltage_min;

    return idle_sec >= conf->bal_idle_delay && status->cell_voltage_max > conf->bal_cell_voltage_min
